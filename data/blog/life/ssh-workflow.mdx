...
The purpose of SSH keys is to enable passkey authentication, which is superior to password authentication, for SSH.

### Generation

Replace the following email with your email

```bash
ssh-keygen -t ed25519 -C "myemail@gmail.com"
# Usually spam enter for defaults is ok, which includes no password
```
...
Note: that improved key generation algorithms may be introduced. Check the below source for most update-to-date commands.
...
Populate `~/.ssh/config` with information. The `~` refers to the home directory of your local device (in my case Windows device). Here is example:
